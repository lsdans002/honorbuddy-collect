        ...
        protected override Composite CreateBehavior()
        {
            return _root ?? (_root =
                new PrioritySelector(
                           ...
                           new Decorator(ret => npcDeadList.Count > 0,
                                new Sequence(
                                    new DecoratorContinue(ret => npcDeadList[0].WithinInteractRange,
                                        new Sequence(
                                            new Action(ret => TreeRoot.StatusText = "Using Item - " + wowItem.Name + " On Mob: " + npcDeadList[0].Name),
                                            new Action(ret => WoWMovement.MoveStop()),
                                            new Action(ret => Thread.Sleep(300)),
                                            new Action(ret => npcDeadList[0].Target()),
                                            new Action(ret => npcDeadList[0].Face()),
                                            new Action(ret => Thread.Sleep(300)),
                                    ...
                                    new DecoratorContinue(ret => !npcDeadList[0].WithinInteractRange,
                                        new Sequence(
                                        new Action(ret => TreeRoot.StatusText = "Moving To Loot - " + npcDeadList[0].Name + " Yards Away: " + npcDeadList[0].Location.Distance(me.Location)),
                                        new Action(ret => Navigator.MoveTo(npcDeadList[0].Location)),
                           ...
                           new Decorator(ret => npcList.Count > 0,
                                new Sequence(
                                    new DecoratorContinue(ret => npcList[0].Location.Distance(me.Location) <= Range && InCombatHPUse > 0 && !me.IsCasting,
                                        new Sequence(
                                            new DecoratorContinue(ret => npcList[0].HealthPercent > InCombatHPUse,
                                                new Sequence(
                                                new Action(ret => TreeRoot.StatusText = "Attacking Mob - Current Health %:" + npcList[0].HealthPercent + " Attacking Until: " + InCombatHPUse),
                                                new Action(ret => WoWMovement.MoveStop()),
                                                new Action(ret => npcDeadList[0].Target()),
                                                new Action(ret => npcDeadList[0].Face()),
                                                new Action(ret => Thread.Sleep(300)),
                                                new Action(ret => SpellManager.Cast(RangeSpell)),
                                                new Action(delegate { return RunStatus.Running; })
                                                )
                                           ),
                                    ...
                                    new DecoratorContinue(ret => npcList[0].Location.Distance(me.Location) <= Range,
                                        new Sequence(
                                            new Action(ret => TreeRoot.StatusText = "Using Item - " + wowItem.Name + " On Mob: " + npcList[0].Name),
                                            new Action(ret => WoWMovement.MoveStop()),
                                            new Action(ret => Thread.Sleep(300)),
