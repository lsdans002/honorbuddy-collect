    ...
    public class VehicleBehavior : CustomForcedBehavior
    {
            ...
            return _root ?? (_root =
                new PrioritySelector(
                           ...
                           new Decorator(ret => !inVehicle,
                                new Sequence(
                                    new DecoratorContinue(ret => npcvehicleList[0].WithinInteractRange,
                                        new Sequence(
                                            new Action(ret => TreeRoot.StatusText = "Mounting Vehicle - " + npcvehicleList[0].Name),
                             ...
                             new Decorator(ret => inVehicle,
                                new Sequence(
                                    new DecoratorContinue(ret => vehicleList[0].Location.Distance(FirePoint) <= 5,
                                        new Sequence(
                                            new Action(ret => TreeRoot.StatusText = "Firing Vehicle - " + vehicleList[0].Name + " Using Spell Index: " + SpellIndex + " Height: " + FireHeight),
                                            new Action(ret => WoWMovement.ClickToMove(TargetPoint)),
                                            new Action(ret => Thread.Sleep(500)),
                                            new Action(ret => WoWMovement.MoveStop()),
                                            new Action(ret => Lua.DoString("VehicleAimRequestNormAngle({0})", FireHeight)),
                                            new Action(ret => Thread.Sleep(300)),
                                            new Action(ret => Lua.DoString("CastPetAction({0})", SpellIndex)),
                                            new Action(ret => Counter++)
                                            )
                                    ),
                                    new DecoratorContinue(ret => vehicleList[0].Location.Distance(FirePoint) > 5,
                                        new Sequence(
                                        new Action(ret => TreeRoot.StatusText = "Moving To FireLocation - Yards Away: " + FirePoint.Distance(vehicleList[0].Location)),
        ...
        WoWPoint moveToLocation
        {
            get
            {
                ...
                Path = Navigator.GeneratePath(vehicleList[0].Location, FirePoint);
                pathIndex = 0;
