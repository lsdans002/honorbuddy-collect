...
namespace AutoEquip
{
    public class AutoEquip : HBPlugin
    {
        ...
        {
            //if (AutoEquipSettings.Instance.ChosenWeightSet == null)
            //    AutoEquipSettings.LoadDefaultSet();
        ...
        private void DoCheck()
        {
            ...
            //Log(true, "Doing check");
            
            RefreshEquippedItems();

            HashSet<WoWItemQuality> equipQualities = AutoEquipSettings.Instance.EquipQualities;
            ...
            foreach (WoWItem item in items)
            {
                ...
                if (_equippedItems.ContainsValue(item)
                    || AutoEquipSettings.Instance.IsItemIgnored(item)
                    || AutoEquipSettings.Instance.IngoreTypes.Contains(item.ItemInfo.InventoryType)
                    || !equipQualities.Contains(item.ItemInfo.Quality)
                    || !ObjectManager.Me.CanEquipItem(item))
                {
                    //if (_equippedItems.ContainsValue(item))
                    //    Log(true, "Ignoring " + item.Name + " because we have it equipped!");
                    //if (AutoEquipSettings.Instance.IsItemIgnored(item))
                    //    Log(true, "Ignoring " + item.Name + " because its ignored!");
                    //if (AutoEquipSettings.Instance.IngoreTypes.Contains(item.ItemInfo.InventoryType))
                    //    Log(true, "Ignoring " + item.Name + " because its an ignored type!");
                    //if (!equipQualities.Contains(item.ItemInfo.Quality))
                    //    Log(true, "Ignoring " + item.Name + " because its not an equippable quality! [" + item.ItemInfo.Quality + "]");
                    //if (!ObjectManager.Me.CanEquipItem(item))
                    //    Log(true, "Ignoring " + item.Name + " because we can't equip it!");
            ...
            //Log(true, "Equippable items: " + equippableNewItems.Count);
        ...
        private static void EquipItemEx(int bagIndex, int bagSlot, int targetSlot)
        {
            Lua.DoString(
        ...
        {
            EquipItemEx(item.BagIndex, item.BagSlot, (int)slot);
        ...
        {
            EquipItemEx(item.BagIndex, item.BagSlot, (int)item.ItemInfo.EquipSlot);
        ...
        private InventorySlot FindBestEquipmentSlot(IEnumerable<InventorySlot> equipSlots, out float lowestItemScore)
        {
            ...
            foreach (InventorySlot inventorySlot in equipSlots)
            {
                ...
                float itemScore = AutoEquipSettings.Instance.ChosenWeightSet.EvaluateItem(equippedItem);
        ...
        private WoWItem FindBestEquipItem(IEnumerable<WoWItem> items, out float bestScore)
        {
            ...
            foreach (WoWItem item in items)
            {
                float itemScore = AutoEquipSettings.Instance.ChosenWeightSet.EvaluateItem(item);
